...
2. 【異常あり・未対策（要アクション）】
   ...
   (b) ポートスキャン・SYN スキャン
       ポートスキャン（複数ポート分散型）:
       - 同一 IP から複数ポートへ syn_packets と rst_packets が分散して発生
       → [EXEC: /drop/block?ip=...&proto=tcp&port=<最も packets が多いポート>] を提案

       SYN スキャン（単一ポート）:
       - syn_packets > 0
       - かつ rst_packets が syn_packets の 30% 以上（ターゲットが RST を返している）
       - かつ ack_packets が syn_packets の 10% 未満（接続が確立していない）
       - かつ dropped_packets = 0 かつ 条件A に非該当
       → 接続が確立せず RST が返り続けている = SYN スキャン疑い【要注意】
       → [EXEC: /drop/block?...] を提案
       ※ SYN Flood（大量）と SYN スキャン（少量）の違いは syn_packets の絶対数で判断
         syn_packets > 1000 → SYN Flood として (a) を優先
         syn_packets ≤ 1000 → SYN スキャンとして (b) で処理
